...
(async function example() {
    ...
    try {
      ...
      await driver.wait(until.elementsLocated(By.id(`password`)), 5000);
      await driver.findElement(By.id(`password`)).sendKeys(`@NewUser123`, Key.RETURN);
      await driver.sleep(5000);
  

      // click button Logout
      await driver.wait(until.elementLocated(By.id(`submit`)), 3000);
      const Logout = await driver.findElement(By.id(`submit`));
